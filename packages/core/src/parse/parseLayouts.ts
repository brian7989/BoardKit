...
export function parseLayouts(value: unknown, path: string, version: number = STATE_VERSION): Result<Layouts, readonly Issue[]> {
  ...
  for (const [key, raw] of Object.entries(object.value)) {
    const tiles = parseLayoutTiles(raw, `${path}.${key}`, version);
...
function parseLayoutTiles(value: unknown, path: string, version: number): Result<readonly LayoutTile[], readonly Issue[]> {
  const array = readArray(value, path);
  if (!array.ok) return err([array.error]);
  const tiles: LayoutTile[] = [];
  for (const [index, entry] of array.value.entries()) {
    const tile = parseLayoutTile(entry, `${path}[${index}]`, version);
